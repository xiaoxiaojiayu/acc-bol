...
NS_LOG_COMPONENT_DEFINE("ndn.ConsumerPconMY");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(ConsumerPconMY);

constexpr double ConsumerPconMY::CUBIC_C;
constexpr uint32_t ConsumerPconMY::BIC_MAX_INCREMENT;
constexpr uint32_t ConsumerPconMY::BIC_LOW_WINDOW;

TypeId
ConsumerPconMY::GetTypeId()
{
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerPconMY")
      .SetGroupName("Ndn")
      .SetParent<ConsumerWindow>()
      .AddConstructor<ConsumerPconMY>()

      .AddAttribute("CcAlgorithm",
                    "Specify which window adaptation algorithm to use (AIMD, BIC, or CUBIC)",
                    EnumValue(CcAlgorithm::AIMD),
                    MakeEnumAccessor(&ConsumerPconMY::m_ccAlgorithm),
      ...
      .AddAttribute("Beta",
                    ...
                    DoubleValue(0.5),
                    MakeDoubleAccessor(&ConsumerPconMY::m_beta),
                    MakeDoubleChecker<double>())

      .AddAttribute("CubicBeta",
                    "TCP CUBIC Multiplicative Decrease factor",
                    DoubleValue(0.8),
                    MakeDoubleAccessor(&ConsumerPconMY::m_cubicBeta),
                    MakeDoubleChecker<double>())
      ...
      .AddAttribute("AddRttSuppress",
                    ...
                    MakeDoubleAccessor(&ConsumerPconMY::m_addRttSuppress),
                    MakeDoubleChecker<double>())

      .AddAttribute("ReactToCongestionMarks",
                    "If true, process received congestion marks",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerPconMY::m_reactToCongestionMarks),
                    MakeBooleanChecker())

      .AddAttribute("UseCwa",
                    "If true, use Conservative Window Adaptation",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerPconMY::m_useCwa),
                    MakeBooleanChecker())

      .AddAttribute("UseCubicFastConvergence",
                    "If true, use TCP CUBIC Fast Convergence",
                    BooleanValue(false),
                    MakeBooleanAccessor(&ConsumerPconMY::m_useCubicFastConv),
                    MakeBooleanChecker())
                    
      .AddAttribute("RandomPrefix", "",
                StringValue(""),
                MakeStringAccessor(&ConsumerPconMY::SetRandomPrefix),
                MakeStringChecker());    ;

  return tid;
}

ConsumerPconMY::ConsumerPconMY()
...
void
ConsumerPconMY::OnData(shared_ptr<const Data> data)
{
...
void
ConsumerPconMY::OnTimeout(uint32_t sequenceNum)
{
  m_flag =1;
  WindowDecrease();
...
void ConsumerPconMY::SetRandomPrefix(std::string random_prefix)  {
...
void ConsumerPconMY::ScheduleNextPacket()  {
      if (randprefix)
       {
