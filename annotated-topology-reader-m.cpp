...
NS_LOG_COMPONENT_DEFINE("AnnotatedTopologyReaderM");

AnnotatedTopologyReaderM::AnnotatedTopologyReaderM(const std::string& path,
                                                   double scale /*=1.0*/)
    : m_path(path),
...
void AnnotatedTopologyReaderM::SetBoundingBox(double ulx, double uly,
...
void AnnotatedTopologyReaderM::SetMobilityModel(const std::string& model) {
    NS_LOG_FUNCTION(this << model);
    m_mobilityFactory.SetTypeId(model);
}

AnnotatedTopologyReaderM::~AnnotatedTopologyReaderM() { NS_LOG_FUNCTION(this); }

Ptr<Node> AnnotatedTopologyReaderM::CreateNode(const std::string name,
                                               uint32_t systemId) {
...
Ptr<Node> AnnotatedTopologyReaderM::CreateNode(const std::string name,
    ...
    Ptr<Node> node = CreateObject<Node>(systemId);
    Ptr<MobilityModel> loc =
        DynamicCast<MobilityModel>(m_mobilityFactory.Create());
...
NodeContainer AnnotatedTopologyReaderM::GetNodes() const { return m_nodes; }

const std::list<TopologyReader::Link>& AnnotatedTopologyReaderM::GetLinks()
    const {
    return m_linksList;
}

NodeContainer AnnotatedTopologyReaderM::Read(void) {
    ifstream topgen;
    ...
    if (topgen.eof()) {
        NS_FATAL_ERROR("Topology file " << GetFileName()
    ...
    while (!topgen.eof()) {
        ...
        node = CreateNode(name, m_scale * longitude, m_scale * latitude,
                          systemId);
        // else {
        //     Ptr<UniformRandomVariable> var =
        //         CreateObject<UniformRandomVariable>();
        //     node = CreateNode(name, var->GetValue(0, 200),
        //                       var->GetValue(0, 200), systemId);
    ...
    while (!topgen.eof()) {
        ...
        AddLink(link);
        NS_LOG_DEBUG("New link " << from << " <==> " << to << " / " << capacity
                                 << " with " << metric << " metric (" << delay
                                 << ", " << maxPackets << ", " << lossRate
                                 << ")");
    }

    NS_LOG_INFO("Annotated topology created with "
                << m_nodes.GetN() << " nodes and " << LinksSize() << " links");
    topgen.close();
...
void AnnotatedTopologyReaderM::AssignIpv4Addresses(Ipv4Address base) {
    Ipv4AddressHelper address(base, Ipv4Mask("/24"));

    BOOST_FOREACH (const Link& link, m_linksList) {
        address.Assign(
            NetDeviceContainer(link.GetFromNetDevice(), link.GetToNetDevice()));
...
void AnnotatedTopologyReaderM::ApplyOspfMetric() {
    BOOST_FOREACH (const Link& link, m_linksList) {
        NS_LOG_DEBUG("OSPF: " << link.GetAttribute("OSPF"));
        uint16_t metric =
            boost::lexical_cast<uint16_t>(link.GetAttribute("OSPF"));

        {
            Ptr<Ipv4> ipv4 = link.GetFromNode()->GetObject<Ipv4>();
            if (ipv4 != 0) {
                int32_t interfaceId =
                    ipv4->GetInterfaceForDevice(link.GetFromNetDevice());
            ...
            Ptr<ndn::L3Protocol> ndn =
                link.GetFromNode()->GetObject<ndn::L3Protocol>();
            if (ndn != 0) {
...

        {
            ...
            if (ipv4 != 0) {
                int32_t interfaceId =
                    ipv4->GetInterfaceForDevice(link.GetToNetDevice());
                NS_ASSERT(interfaceId >= 0);
            ...
            Ptr<ndn::L3Protocol> ndn =
                link.GetToNode()->GetObject<ndn::L3Protocol>();
            if (ndn != 0) {
                shared_ptr<ndn::Face> face =
...
void AnnotatedTopologyReaderM::ApplySettings() {
#ifdef NS3_MPI
    ...
    BOOST_FOREACH (Link& link, m_linksList) {
        // cout << "Link: " << Findlink.GetFromNode () << ", " << link.GetToNode
        // () << endl;
        string tmp;
...
void AnnotatedTopologyReaderM::SaveTopology(const std::string& file) {
    ...
    os << "# any empty lines and lines starting with '#' symbol is ignored\n"
       ...
       << "# The file should contain exactly two sections: router and link, "
          "each starting with the "
          "corresponding keyword\n"
       << "\n"
       << "# router section defines topology nodes and their relative "
          "positions (e.g., to use in "
          "visualizer)\n"
       << "router\n"
       << "\n"
       << "# each line in this section represents one router and should have "
          "the following data\n"
       << "# node  comment     yPos    xPos\n";

    for (NodeContainer::Iterator node = m_nodes.Begin(); node != m_nodes.End();
         node++) {
    ...
    os << "# link section defines point-to-point links between nodes and "
       ...
       << "# queue:  MaxPackets for transmission queue on the link (both "
          "directions)\n"
       << "# error:  comma-separated list, specifying class for ErrorModel and "
          "necessary attributes\n";

    for (std::list<Link>::const_iterator link = m_linksList.begin();
         link != m_linksList.end(); link++) {
...
class name_writer {
  public:
    ...
    void operator()(std::ostream& out, const VertexOrEdge& v) const {
        // out << "[label=\"" << names[v] << "\",style=filled,fillcolor=\"" <<
        // colors[v] << "\"]";
        out << "[shape=\"circle\",width=0.1,label=\"\",style=filled,fillcolor="
               "\"green\"]";
    }
...
template <class Names>
inline name_writer<Names> make_name_writer(Names n) {
    return name_writer<Names>(n);
}

/// @endcond

void AnnotatedTopologyReaderM::SaveGraphviz(const std::string& file) {
    typedef boost::adjacency_list_traits<boost::setS, boost::setS,
                                         boost::undirectedS>
        Traits;
    ...
    for (std::list<Link>::const_iterator link = m_linksList.begin();
         link != m_linksList.end(); link++) {
        node_map_t::iterator from =
            graphNodes.find(Names::FindName(link->GetFromNode()));
        node_map_t::iterator to =
            graphNodes.find(Names::FindName(link->GetToNode()));
    ...
    boost::property_map<Graph, boost::vertex_name_t>::type names =
