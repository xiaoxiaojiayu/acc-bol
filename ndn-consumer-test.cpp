  ...
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerTest")
      ...
      .AddAttribute("Randomize",
                    ...
                    StringValue("none"),
                    MakeStringAccessor(&ConsumerTest::SetRandomize, &ConsumerTest::GetRandomize),
                    MakeStringChecker())

      .AddAttribute("MaxSeq", "Maximum sequence number to request",
                    IntegerValue(std::numeric_limits<uint32_t>::max()),
                    MakeIntegerAccessor(&ConsumerTest::m_seqMax), MakeIntegerChecker<uint32_t>())
...
void
ConsumerTest::SetRandomize(const std::string& value)
{
  if (value == "uniform") {
...
std::string
ConsumerTest::GetRandomize() const
{
  return m_randomType;
}
