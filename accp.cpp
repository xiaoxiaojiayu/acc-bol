...
NS_LOG_COMPONENT_DEFINE("ndn.ConsumerACCP");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(ConsumerACCP);

constexpr double ConsumerACCP::CUBIC_C;
constexpr uint32_t ConsumerACCP::BIC_MAX_INCREMENT;
constexpr uint32_t ConsumerACCP::BIC_LOW_WINDOW;

TypeId
ConsumerACCP::GetTypeId()
{
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerACCP")
      .SetGroupName("Ndn")
      .SetParent<ConsumerWindow>()
      .AddConstructor<ConsumerACCP>()

      .AddAttribute("CcAlgorithm",
                    "Specify which window adaptation algorithm to use (AIMD, BIC, or CUBIC)",
                    EnumValue(CcAlgorithm::AIMD),
                    MakeEnumAccessor(&ConsumerACCP::m_ccAlgorithm),
      ...
      .AddAttribute("Beta",
                    ...
                    DoubleValue(0.5),
                    MakeDoubleAccessor(&ConsumerACCP::m_beta),
                    MakeDoubleChecker<double>())

      .AddAttribute("CubicBeta",
                    "TCP CUBIC Multiplicative Decrease factor",
                    DoubleValue(0.8),
                    MakeDoubleAccessor(&ConsumerACCP::m_cubicBeta),
                    MakeDoubleChecker<double>())
      ...
      .AddAttribute("AddRttSuppress",
                    ...
                    MakeDoubleAccessor(&ConsumerACCP::m_addRttSuppress),
                    MakeDoubleChecker<double>())

      .AddAttribute("ReactToCongestionMarks",
                    "If true, process received congestion marks",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerACCP::m_reactToCongestionMarks),
                    MakeBooleanChecker())

      .AddAttribute("UseCwa",
                    "If true, use Conservative Window Adaptation",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerACCP::m_useCwa),
                    MakeBooleanChecker())

      .AddAttribute("UseCubicFastConvergence",
                    "If true, use TCP CUBIC Fast Convergence",
                    BooleanValue(false),
                    MakeBooleanAccessor(&ConsumerACCP::m_useCubicFastConv),
                    MakeBooleanChecker())
                    
      .AddAttribute("RandomPrefix", "",
                StringValue(""),
                MakeStringAccessor(&ConsumerACCP::SetRandomPrefix),
                MakeStringChecker());    ;

  return tid;
}

ConsumerACCP::ConsumerACCP()
  ...
  , ssflag(1)
{
  avg_rtt = MilliSeconds(40);
}

void
ConsumerACCP::OnData(shared_ptr<const Data> data)
{
  ...
  double mark =0;
  mark = data->getCongestionMark();
  NS_LOG_DEBUG("mark: " << mark);
  avg_queue = accp_g * mark + (1-accp_g) * avg_queue;
  
  avg_rtt = accp_g * m_rtt->GetCurrentEstimate() + (1-accp_g) * avg_rtt;
  

  
  int64_t cur_time = Simulator::Now().GetNanoSeconds();
  //NS_LOG_DEBUG("cur_time: " << cur_time << ", updatetime: " << updatetime << ", avg_rtt: " << avg_rtt.GetSeconds()*1000);
  if (ssflag)
  {
    WindowIncrease();
  }
  else if(cur_time-updatetime > avg_rtt.GetSeconds()*1000000000)
  {

  if (avg_queue > Q_max) {
    ACCPDecrease();
  }
  else if(avg_queue>Q_heavybusy){
    ACCPNochange();
  }
  else if(avg_queue>Q_busy){
    ACCPIncreaseEI();
  }
  else
  {
...
void
ConsumerACCP::OnTimeout(uint32_t sequenceNum)
{
  m_flag =1;
  ssflag =0;
  //WindowDecrease();
...
ConsumerACCP::WindowIncrease()
{
  ...
  // if (m_ccAlgorithm == CcAlgorithm::CUBIC) {
  //   CubicIncrease();
  // }
...
void ConsumerACCP::ACCPDecrease()
{
    m_window = m_window * accp_gamma;
    updatetime = Simulator::Now().GetNanoSeconds();
    NS_LOG_DEBUG("ACCPDecrease " );
   
}

void ConsumerACCP::ACCPIncreaseEI()
{
    
   
      m_window = m_window * (1.0 + accp_alpha);
      updatetime = Simulator::Now().GetNanoSeconds();
      NS_LOG_DEBUG("ACCPIncreaseEI " );
    
}

void ConsumerACCP::ACCPIncreaseAI()
{
    
   m_window = m_window + accp_beta;
   updatetime = Simulator::Now().GetNanoSeconds();
   NS_LOG_DEBUG("ACCPIncreaseAI" );
}

void ConsumerACCP::ACCPNochange()
{
    
   m_window = m_window ;
   updatetime = Simulator::Now().GetNanoSeconds();
   NS_LOG_DEBUG("ACCPNochange" );
}

void
ConsumerACCP::WindowDecrease()
{
...
void ConsumerACCP::SetRandomPrefix(std::string random_prefix)  {
...
void ConsumerACCP::ScheduleNextPacket()  {
      if (randprefix)
       {
